...
fn initially_displays_char_as_hidden() {
    let secret_char = SecretChar::from(Char::from('a').unwrap());
...
fn displays_revealed_char_as_uppercase() {
    let mut secret_char = SecretChar::from(Char::from('a').unwrap());
...
fn displays_non_case_convertible_char_as_is() {
    let mut secret_char = SecretChar::from(Char::from('ß').unwrap());
